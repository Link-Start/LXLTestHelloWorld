...
- (void)setModel:(SettingTableViewBindActionModel *)model {
    ...
    self.rightLabel.hidden = !(model.rightLabelStr.length>0);
}
...
- (void)addContentViewSubViewsLayout {
    ...
    self.rightLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-15-150, 10, 150, 30);
    self.rightImgV.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-15-20, 10, 20, 20);
}
